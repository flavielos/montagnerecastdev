...
const replyMessage = (message) => {
  ...
  .then(result => {
    ...
    */
	...
	// get prenom
	if (result.action && result.action.slug == 'nom' && result.action.done){
	...
	// get age
	if (result.action && result.action.slug == 'age'){
	...
	if (result.action && result.action.slug == 'nombre-age' && result.action.done){
	...
	// get niveau physique
	if (result.action &&  result.action.slug == 'niveau-physique'){
		if(result.action.done){
	...
	// get budget
	if (result.action && result.action.slug == 'budget'){
		if (result.action.done){
	...
	if (result.action && result.action.slug == 'nombre-budget' && result.action.done){
	...
	// get eloignement
	if (result.action && result.action.slug == 'eloignement'){
		if(result.action.done){
			var dist;
	...
	if (result.action && result.action.slug == 'pas-de-preference-eloignement'){
		var nb = 0;
	...
	if (result.action &&  result.action.slug == 'niveau-randonneur'){
		if(result.action.done){
	...
	//get détails
	if (result.action && result.action.slug == 'details'){
		if (result.action.done){
	...
	// Reco erronée
	if (result.action && result.action.slug == 'reco-invalide'){
	...
	// Get ajustement
	if (result.action && result.action.slug == 'ajuster-reco'){
